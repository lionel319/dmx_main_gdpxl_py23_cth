...
class CallGraphVisitor(object):
    ...
    def visitModule(self, node):
        verbose_output("inside visitModule({})".format(node))
    ...
    def visitClass(self, node):
        ...
        verbose_output("inside visitClass({})".format(node))

        ### Skip Exception Classes
        if self.options.skipErrorClasses and node.name.endswith("Error"):
            return
    ...
    def visitFunction(self, node):
        if not self.options.visitfunctions:
            return
    ...
    def visitImport(self, node):
        verbose_output("inside visitImport({})".format(node))
        for import_item in node.names:
            ### Added by Lionel on 02 Aug 2016
            ### this fix the problem of pyan.py not generating edge when 
            ### import dmx.ecolib.* statement is used. Previously it was working when 
            ### from dmx.ecolib.* import * statement is used.
            ...
            verbose_output("- from:{} to:{} tgt_name:{}, import_item:{}".format(from_node, to_node, tgt_name, import_item))
    ...
    def visitFrom(self, node):
        ...
        verbose_output("inside visitFrom({}), tgt_name:{}".format(node, tgt_name))
        if self.add_uses_edge(from_node, to_node):
...
def main():
    ...
    parser.add_option("--visitfunctions", 
            action="store_true", default=False,
            help="Visit all the functions/methods so that they are included in the graph.")

    parser.add_option("--skipErrorClasses", 
            action="store_true", default=False,
            help="Ignore all classes with *Error(Exception) name.")
