...
class FieldChoicePlugin(DatabrowsePlugin):
    ...
    def model_index_html(self, request, model, site):
        ...
        return mark_safe(u'<p class="filter"><strong>View by:</strong> %s</p>' % \
            u', '.join(['<a href="fields/%s/">%s</a>' % (f.name, force_unicode(capfirst(f.verbose_name))) for f in list(fields.values())]))

    def urls(self, plugin_name, easy_instance_field):
        if easy_instance_field.field in list(self.field_dict(easy_instance_field.model.model).values()):
            field_value = smart_str(easy_instance_field.raw_value)
            return [mark_safe(u'%s%s/%s/%s/' % (
                easy_instance_field.model.url(),
                plugin_name, easy_instance_field.field.name,
                urllib.parse.quote(field_value, safe='')))]
