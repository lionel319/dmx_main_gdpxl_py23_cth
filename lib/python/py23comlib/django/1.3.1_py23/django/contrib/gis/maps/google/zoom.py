...
class GoogleZoom(object):
    ...
    def __init__(self, num_zoom=19, tilesize=256):
        ...
        for i in range(num_zoom):
            ...
            self._radpp.append(old_div(z, (2 * pi))) # radians per pixl
            self._npix.append(old_div(z, 2)) # number of pixels to center of tile
    ...
    def lonlat_to_pixel(self, lonlat, zoom):
        ...
        px_y = round(npix + (0.5 * log(old_div((1 + fac),(1 - fac))) * (-1.0 * self._radpp[zoom])))
    ...
    def pixel_to_lonlat(self, px, zoom):
        ...
        lon = old_div((px[0] - npix), self._degpp[zoom])

        # Calculating the latitude value.
        lat = RTOD * ( 2 * atan(exp(old_div((px[1] - npix), (-1.0 * self._radpp[zoom])))) - 0.5 * pi)
    ...
    def tile(self, lonlat, zoom):
        ...
        delta = old_div(self._tilesize, 2)
