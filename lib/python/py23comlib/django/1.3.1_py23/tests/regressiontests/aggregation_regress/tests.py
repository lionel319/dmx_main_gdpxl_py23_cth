...
class AggregationTests(TestCase):
    ...
    def test_aggregate_fexpr(self):
        ...
        qs = Publisher.objects.annotate(num_books=Count('book')).filter(num_books__lt=old_div(F('num_awards'),2)).order_by('name').values('name','num_books','num_awards')
        self.assertQuerysetEqual(
            qs, [
        ...
        qs = Publisher.objects.annotate(num_books=Count('book')).exclude(num_books__lt=old_div(F('num_awards'),2)).order_by('name').values('name','num_books','num_awards')
        self.assertQuerysetEqual(
            qs, [
