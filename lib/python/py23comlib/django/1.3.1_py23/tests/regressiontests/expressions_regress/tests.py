...
class ExpressionOperatorTests(TestCase):
    ...
    def test_lefthand_division(self):
        ...
        Number.objects.filter(pk=self.n.pk).update(integer=old_div(F('integer'), 2),
                                              float=F('float') / 42.7)
    ...
    def test_right_hand_division(self):
        ...
        Number.objects.filter(pk=self.n.pk).update(integer=old_div(640, F('integer')),
                                              float=42.7 / F('float'))
...
class FTimeDeltaTests(TestCase):
    ...
    def test_delta_invalid_op_div(self):
        raised = False
        try:
            r = repr(Experiment.objects.filter(end__lt=old_div(F('start'),self.deltas[0])))
        except TypeError:
            raised = True
