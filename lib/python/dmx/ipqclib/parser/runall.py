#!/usr/bin/env python
"""Parser for run-all sub-command"""
import argparse
...
def get_runall_parser(action, subparsers, group_mandatory, group_exclusive, group_optional):
    """Create parser for run-all sub-command"""
    run_all = subparsers.add_parser(action, formatter_class=argparse.RawTextHelpFormatter, \
            add_help=False, description='Creates IPQC environment in reldoc/ipqc/<milestone>. \
            Executes the tests for the given milestone and generates a report.')

    # mandatory arguments
    group_mandatory[action] = run_all.add_argument_group('mandatory arguments')
    ...
    # optional arguments
    group_optional[action] = run_all.add_argument_group('optional arguments')
    ...
    group_optional[action].add_argument('--init-file', dest='initfile', action='store', \
            default=None, \
            required=False, metavar='<initfile_path>', type=is_valid_file, \
            help='IPQC configuration file path.')
    ...
    group_optional[action].add_argument('--requalify', dest='requalify', action='store_true', \
            default=False, \
            required=False, help='By default, for immutable deliverable BOM, IPQC will not \
            run the check. \
            It assumes it has been run prvious release. If --requalify is invoked, IPQC will \
            run the check on immutable BOM')
    ...
    group_optional[action].add_argument('--report-template', dest='report_template', \
            action='store', \
            type=is_valid_template_value, default=_VIEW, required=False, help='template of \
            ipqc report.\n\tview: report by view (front-end, back-end, timing) \
    ...
    group_optional[action].add_argument('--sendmail', dest='sendmail', action='store_true', \
            ...
            help='send report by email at the end of the execution.')
    group_optional[action].add_argument('--filter-status', dest='filter_status', \
            ...
            choices=_FILTER_OPTIONS, help='Specify the status you want to filter out in the HTML\
            report. Value = {}' .format(_FILTER_OPTIONS))
    ...
    group_optional[action].add_argument('--recipients', dest='recipients', action='store', \
            ...
            help="File containing list of recipient email the report is sent to \
            (use one email per line). --sendmail needs to be invoked\nExample: \
    ...
    group_optional[action].add_argument('--sync', dest='sync', action='store_true', \
            default=False, required=False, help='By default, IPQC does not sync the \
            workspace. You can use --sync to make sure data are up-to-date.')
    ...
    group_optional[action].add_argument('--flow', dest='flow', action='store', \
            required=False, default="flat", choices=["flat", "hier"], help=argparse.SUPPRESS)
    # --ciw option
    ...
    group_exclusive[action]['hierarchy'] = run_all.add_mutually_exclusive_group(required=False)
    group_exclusive[action]['hierarchy'].add_argument('--no-hierarchy', dest='no_hierarchy', \
            action='store_true', default=False, \
            required=False, help='Tabulate the data for all topcells embedded in the top \
            level configuration.')
    ...
    group_exclusive[action]['checkin'] = run_all.add_mutually_exclusive_group(required=False)
    group_exclusive[action]['checkin'].add_argument('--check-in', dest='checkin', \
            action='store_true', default=False, required=False, help='check-in deliverable \
            manifest into DM system after check execution.')
    group_exclusive[action]['checkin'].add_argument('--check-in-only-pass', \
            ...
            execution and only for pass or waive status. Files are reverted in case of \
            failure.')
    ...
    return (run_all, group_mandatory, group_exclusive, group_optional)
