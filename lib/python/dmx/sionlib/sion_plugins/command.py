...
from dmx.sionlib.sion_utils import run_as_headless_user, run_command, print_errors
from dmx.sionlib.sion_utils import get_var_prot_group, get_proj_prot_group
...
class CommandRunner(Runner):
    ...
    def __init__(self, icm_command):
        ...
        if self.icm_command.startswith("icmp4 ") or self.icm_command.startswith("ipqc "):
            icmp4_cmd = self.icm_command.split()[1]
            if (icmp4_cmd not in ICM_READONLY_CMDS) and (not self.icm_command.startswith("ipqc ")):
                raise CommandError("{0} is not allowed to be run by sion. Only IPQC and selected ICM commands may be run by sion.".format(icmp4_cmd))
            ...
            raise CommandError("ICM command must be annotated as a string and always start with icmp4. Example:\n  sion command -c \'icmp4 dirs //depot/icm/proj/*\"")
    ...
    def run(self):
        ...
        ret = run_as_headless_user(command = self.command, icm_command = self.icm_command)
...
class Command(Command):
    ...
    def command(cls, args):
        ...
        print("Requested command: %s" % icm_command)
