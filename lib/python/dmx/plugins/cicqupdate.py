...
class CicqUpdate(Command):
    '''plugin for "dmx cicq update"'''
    ...
    def add_args(cls, parser):
        ...
        parser.add_argument('-t', '--thread', required=True, nargs='+')
        parser.add_argument('-c', '--cfgfile', required=False, default='')
    ...
    def extra_help(cls):
        ...
        extra_help = '''\
            "dmx cicq update" is used align all the cicq backend boms with the given bom.
    ...
    def command(cls, args):
        ...
        if len(threads) == 1:
            ci = dmx.abnrlib.flows.cicqupdate.CicqUpdate(project, ip, bom, suffix=threads[0], reuse_immutable=True, cfgfile=cfgfile, init=False, dryrun=dryrun)
            ret = ci.run()
            ...
            for thread in threads:
                cmd = '{} cicq update -p {} -i {} -b {} -t {} --debug'.format(dmxexe, project, ip, bom, thread)
                if preview:
                    cmd += ' -n'
                if cfgfile:
                    cmd += ' --cfgfile {}'.format(cfgfile)
