#!/usr/bin/env python
'''
$Header: //depot/da/infra/dmx/main_gdpxl_py23_cth/lib/python/dmx/plugins/workspacelocaledit.py#1 $
$Change: 7411538 $
...
class WorkspaceLocalEdit(Command):
    ...
    def command(cls, args):
        ...
        if args.files and len(args.deliverables) is not 1:
            raise Exception("Multiple deliverables is not allowed with -f option.") 

        wu = dmx.abnrlib.flows.workspacelocaledit.WorkspaceLocalEdit(args.wsname, ip=args.ip, deliverables=args.deliverables, files=args.files, preview=args.preview)
        ret = wu.run()
