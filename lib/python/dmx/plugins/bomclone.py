...
from dmx.abnrlib.flows.cloneconfigs import CloneConfigs
from dmx.abnrlib.flows.cloneconfigswithrels import *
from dmx.abnrlib.flows.cloneconfigsemptybranch import CloneConfigsEmptyBranch
from dmx.abnrlib.icm import ICManageCLI
from dmx.utillib.utils import get_ww_details, normalize_config_name
...
class BomClone(Command):
    ...
    def command(cls, args):
        ...
        # cloneconfigs arguments
        clone_simple = False
        clone_immutable = args.clone_immutable
        reuse = args.reuse
        deliverable = args.deliverable
        
        # Default prism dstbom
        # https://jira.devtools.intel.com/browse/PSGDMX-2152
        if not args.dstbom:
            logger.info('No --dstbom detected, getting default bom name from source bom')    
            (year, ww, day) = get_ww_details()
            bom = normalize_config_name(bom)
            prism_bom = '{0}__{1}ww{2}{3}'.format(bom, year, ww, day)
            dstbom = ICManageCLI().get_next_snap_config(project, ip, prism_bom)
            logger.info('Destination bom : {}'.format(dstbom))
        else:
            dstbom = args.dstbom
            logger.info('Destination bom : {}'.format(dstbom))
 
        # cloneconfigswithrels arguments
        replace_with_rels = args.replace_with_rels
        ...
        # emptybranch arguments
        emptybranch = args.emptybranch

        if emptybranch:
            clone = CloneConfigsEmptyBranch(project, ip, bom, dstbom, libtype=deliverable, preview=preview)

        elif not replace_with_rels:
            clone = CloneConfigs(project, ip, bom, dstbom, libtype=deliverable,
                                 clone_simple=clone_simple, clone_immutable=clone_immutable,
                                 reuse=reuse, preview=preview)
        else:
            # milestone and thread are compulsory arguments for cloneconfigswithrels
            # that's why we checked here
            if not milestone:
                raise CloneConfigsWithRelsError('Please provide milestone of the REL to replace.')
            if not thread:
                raise CloneConfigsWithRelsError('Please provide thread of the REL to replace.')
                       
            clone = CloneConfigsWithRels(project, ip, bom, dstbom, milestone,
                                         thread, stop, use_labeled, preview)
