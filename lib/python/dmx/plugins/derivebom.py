...
'''
    ...
    def command(cls, args):
        ...
        # branchlibraries arguments
        reuse = args.reuse
        exact = args.exact
        hierarchy = args.hierarchy
        derivative = args.derivative

        # branchlibrary arguments
        deliverable = args.deliverable

        ### Allow admins to derive bom from mutables.
        if not bom.startswith('REL') and not bom.startswith('snap') and not bom.startswith('PREL') and not dmx.utillib.admin.is_admin():
            raise Exception('Source BOM must be immutable (REL/PREL/snap)')

        if deliverable:
            # we don't allow branching of library to another project or variant
            target_project = target_ip = target_bom = None

            # branching only a single library
            branch = BranchLibrary(project, ip, deliverable, bom, thread,
                                   target_project=target_project, target_variant=target_ip,
                                   target_config=target_bom, description=desc,
                                   directory=directory, 
                                   preview=preview, called_from_branch_libraries=exact)
        else:            
            # we don't allow users to select which deliverable to branch, all deliverable branched by default
            deliverables = None
            # We don't allow users to name their branch without normalizing the name
            # exact = False

            # branching a variant
            branch = BranchLibraries(project, ip, bom, thread, libtypes=deliverables,
                                     description=desc, reuse=reuse, exact=exact,
                                     directory=directory,
                                     preview=preview, hierarchy=hierarchy, derivative=derivative)
        return branch.run()
