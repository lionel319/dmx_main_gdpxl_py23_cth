#!/usr/bin/env python
'''
$Header: //depot/da/infra/dmx/main_gdpxl_py23_cth/lib/python/dmx/plugins/roadmap.py#1 $
$Change: 7411538 $
...
from dmx.abnrlib.flows.listecosphere import ListEcosphere
...
class Roadmap(Command):
    ...
    def command(cls, args):
        ...
        # Flow and subflow are removed as moving forward, these terms will no longer exist
        '''
        flow = args.flow
        ...
        flow = None
        subflow = None
        # History is not supported for LEGACY mode
        history = None
        ...
        # admin only option
        if is_admin():
            testdata = not args.testdata
        else:
            testdata = False            
        production = testdata

        ret = 1
        list = ListEcosphere(family, project, product, revision, ip, 
                                 ip_type, milestone, deliverable, flow, subflow,
                                 check, cell, unneeded, history, bom, view, roadmap,
                                 production=production, thread=thread)
        ret = list.run()
