...
class JUnitXmlResult(unittest.TestResult):
    ...
    def _test_case_string(self, test):
        duration = self._duration(self._test_start); _ = duration
        test_id = test.id()
        ...
        self._results.append('<testcase classname="%s" name="%s" '
             % (_escape_attr(classname), _escape_attr(name)))
# TO_DO: Put back            'time="%0.3f"' % (_escape_attr(classname), _escape_attr(name), duration))
    ...
    def stopTestRun(self):
        ...
        _ = duration
# TO_DO: Put back        self._stream.write('<testsuite errors="%d" failures="%d" name="" '
#            'tests="%d" time="%0.3f">\n' % (len(self.errors),
