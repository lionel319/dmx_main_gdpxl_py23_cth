...
# $Header: //depot/da/infra/dmx/main_gdpxl_py23_cth/lib/python/dmx/dmlib/CheckerBase.py#3 $
...
class CheckerBase(with_metaclass(ABCMeta, object)):
    ...
    def __init__(self, icmws, ip, cell):
        self._ip = ip
        self._cell_name = cell
        self._icmws = icmws
        self._workspacePath = self._icmws.path
    ...
    @classmethod
    def isMilkywayLibrary(cls, path):
        '''Return true if the specified path is an existing, readable Milkyway
        ...
        False
        '''
        catalogFile = os.path.join(path, 'lib')
        return cls._isFileReadable(catalogFile)

    def _checkMilkywayLibrary(self, path, deliverableName, itemName):
        '''Perform a full type check on the specified path to determine
        ...
        '''
        catalogFile = os.path.join(path, 'lib')
        return self._checkFile(catalogFile, deliverableName, itemName)
        ...
        * Within the current workspace, which is presumed to be the current \
        ...
        if self._hasLink(path):
            ...
            if not self.naa.is_path_naa_path(path) and not self.cache.is_path_cache_path(path):
                self._errors.append("In '{}', {} '{}' contains symbolic link(s)."
                                    "".format(deliverableName,
                                              itemName,
                                              path))
                isPassing =  False
        if ((not self._isAllowedToBeOutsideWorkspace(path)) and
                 self._isOutsideWorkspace(path, workspacePath=self._workspacePath)):
            self._errors.append("In '{}', {} '{}' is outside the current workspace '{}'."
                                "".format(deliverableName,
                                          itemName,
