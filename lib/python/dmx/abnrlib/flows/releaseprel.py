...
from dmx.utillib.utils import format_configuration_name_for_printing, split_pvlc, get_thread_and_milestone_from_prel_config
...
class ReleasePrelError(Exception): pass

class ReleasePrel(object):
    '''
    Runs the releaseview abnr subcommand
    '''

    def __init__(self, project, variant, prel, config, milestone, thread,
        ...
        if not self.project:
            ...
            for arc_project in arc_projects:
                if self.cli.variant_exists(arc_project, self.variant):
                    ...
                    break
            if not self.project:
                raise ReleasePrelError('Variant {0} is not found in projects: {1}'.format(self.variant, arc_projects))
        else:
            # Make sure the project exists
            if not self.cli.project_exists(self.project):
                raise ReleasePrelError("{0} is not a valid project".format(self.project))
            # Make sure the variant exist
            if not self.cli.variant_exists(self.project, self.variant):
                raise ReleasePrelError("{0}/{1} is not a valid variant".format(self.project, self.variant))
        ...
        if not self.cli.config_exists(self.project, self.variant, self.config):
            raise ReleasePrelError('{0} does not exist'.format(format_configuration_name_for_printing(self.project, self.variant, self.config)))
        ...
        # Ensure prel is valid
        try:
            prelobj = family.get_prel(self.prel)
        except Exception as e:
            self.logger.error('Prel ({}) does not exist'.format(self.prel))
            raise ReleasePrelError(e)

        ip = family.get_ip(self.variant, self.project)
        libtypes_to_release = ip.get_deliverables(milestone=self.milestone, local=False, bom=self.config, roadmap=roadmap, prels=[self.prel])
        ...
        self.logger.info("Libtypes to release for prel:{} = {}".format(self.prel, self.deliverables))
    ...
    def run(self):
        ...
        released_simple_configs = [x for x in self.source_cfg.configurations if not x.is_config() and x.is_preleased()]
        ...
        for simple_config in released_simple_configs:
            thread, milestone = get_thread_and_milestone_from_prel_config(simple_config.name)
            if milestone < self.milestone:
                errors.append(simple_config.get_full_name())

        # Every sub-composite configs must be released
        unreleased_composite_configs = [x.get_full_name() for x in self.source_cfg.configurations if x.is_config() and not x.is_preleased()]
        if unreleased_composite_configs:
            error_msg = 'The following BOMs need to be REL/PREL or \'dmx release\' will not be able to release this IP:\n'
            for unreleased_composite_config in sorted(unreleased_composite_configs):
                error_msg = '{}\t{}\n'.format(error_msg, unreleased_composite_config)
            raise ReleasePrelError(error_msg) 

        # For released composite configs, they must satisfy the milestone requirement
        if not self.skipmscheck:
            released_composite_configs = [x for x in self.source_cfg.configurations if x.is_config() and x.is_preleased()]
            # http://pg-rdjira:8080/browse/DI-653
            # Ensure that every released config's milestone is greater or equal to the milestone to release
            for composite_config in released_composite_configs:
                thread, milestone = get_thread_and_milestone_from_prel_config(composite_config.name)
                if milestone < self.milestone:
        ...
        unreleased_simple_configs = [x for x in self.source_cfg.configurations if not x.is_config() and not x.is_preleased()]          
        ...
        if unreleased_simple_configs:
            ...
            self.cloned_config = self.source_cfg.clone('{}_releaseprel_{}'.format(self.config, datetimestr))
    ...
    def release_simple_configs(self, unreleased_simple_configs):
        ...
        for unreleased_simple in unreleased_simple_configs:
            ...
            if unreleased_simple.libtype != 'ipspec':
                ipspec = self.cloned_config.search(project=unreleased_simple.project, variant='^{0}$'.format(unreleased_simple.variant), libtype='ipspec')

                if not ipspec:
                    raise ReleasePrelError('Cannot find ipspec for {0}'.format(unreleased_simple.get_full_name()))
                else:
                    ipspec = ipspec[0]
                    # Make sure we haven't extracted the wrong ipspec config name
                    if ipspec.variant != unreleased_simple.variant:
                        raise ReleasePrelError('Got a bad ipspec for {0}. Found ipspec {1}'.format(unreleased_simple.get_full_name(), ipspec.get_full_name()))
            ...
            variant_config = self.cloned_config.search(project=unreleased_simple.project, variant='^{0}$'.format(unreleased_simple.variant))[0]

            prel_libtype = '{}:{}'.format(self.prel, unreleased_simple.libtype)
            mp_args.append([unreleased_simple.project, unreleased_simple.variant,
                            prel_libtype, variant_config.config,
        ...
        for result in results:
            if not result['success']:
                raise ReleasePrelError('Problem releasing {0}/{1}:{2}@{3}'.format(
                    result['project'], result['variant'], result['libtype'],
                    result['original_config']
                ))
            else:
                prel, libtype = result['libtype'].split(':')
                
                # Only get from IC Manage if we're not in preview mode
                if not self.preview:
                    new_rel_cfg = ConfigFactory.create_from_icm(result['project'], result['variant'], result['released_config'], libtype=libtype, preview=self.preview)
                else:
                    # Create a fake released config with false data. We're only in preview mode
                    # so this should suffice
                    new_rel_cfg = SimpleConfig(result['released_config'], result['project'], result['variant'], libtype, 'dev', '1', preview=self.preview, use_db=False)
    ...
    def release_all_composite_configs(self):
        ...
        :rtype: CompositeConfig
        '''
        configs_to_release = self.source_cfg.get_configs_ready_for_prelease()
            ...
            while configs_to_release:
                ...
                for unreleased_config in configs_to_release:
                    ...
                    mp_args.append([unreleased_config.project, unreleased_config.variant, sub_configs,
                                    ...
                                    self.preview, [],
                                    False, [], self.syncpoint, self.skipsyncpoint, 
                                    self.skipmscheck, self.prel, self.regmode])
                ...
                for result in results:
                    if result['success']:
                        if not self.preview:
                            ...
                            # Create a fake released config with false data. We're only in preview mode
                            # so this should suffice
                            new_rel_cfg = CompositeConfig(result['released_config'], result['project'], result['variant'], [], preview=self.preview)
                    else:
                        raise ReleasePrelError('Problem releasing variant for {0}/{1}'.format(result['project'], result['variant']))
    
                    # Did we just release root?
                    if len(configs_to_release) == 1 and configs_to_release[0] == self.source_cfg:
                        self.source_cfg = new_rel_cfg
                    else:
                        self.source_cfg.replace_all_instances_in_tree(new_rel_cfg.project, new_rel_cfg.variant, new_rel_cfg)
