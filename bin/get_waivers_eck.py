...
from dmx.tnrlib.dashboard_query_es import DashboardQueryEs
...
def main():
    ...
    global password
    username = 'eck-admin'
    password = 'RmK8oydXqbvT56rL'
    host = 'psgdmx.eck1es.intel.com'
    port = '9443'
    

    d = DashboardQueryEs(userid=username, password=password, host=host, port=port)
    ...
    with open(waiver_file, 'w', encoding='utf-8-sig') as csvfile:
        writer = csv.writer(csvfile)
        if args.forrel:
            writer.writerow(['#Variant', 'Flow', 'Subflow', 'Waiver-reason', 'Error'])
        else:

            header = ['Project', 'Variant', 'Libtype', 'Topcell', 'Flow-Libtype', 'Releaser','Flow','Subflow','Waiver-creator','Waiver-reason','Error']
            b_header = [str.encode(h) for h in header]
            print(b_header)
            print(type(b_header[0]))
    ...
    if args.libtype:
        ...
        waivers = d.get_waived_errors_from_pvlc(args.project, args.variant, args.libtype, args.config)
            ...
            if c.is_released():
                ...
                if args.variants_filter == None or variant in args.variants_filter:
                    if args.libtypes_filter == None or libtype in args.libtypes_filter or libtype == "None":
                        waivers = d.get_waived_errors_from_pvlc(project, variant, libtype, config)
                        write_to_csv(waivers, waiver_file, args.libtypes_filter, args.forrel)
                    else:
                        logger.info("- skipped.")
                else:
...
if __name__ == '__main__':
    ...
    print(type(ssl.PROTOCOL_TLSv1))
    print(type(ssl.PROTOCOL_TLS))
