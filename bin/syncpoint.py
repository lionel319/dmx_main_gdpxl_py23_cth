#!/usr/bin/env python
'''
$Header: //depot/da/infra/dmx/main_gdpxl_py23_cth/bin/syncpoint.py#1 $

Description:  syncpoint: Tool to create/delete syncpoint to track specific goal of releases
...
def main():
    ...
    parser = argparse.ArgumentParser(formatter_class=CustomHelpFormatter, description="Syncpoint tool to help track specific goal of a set of releases. For more information, visit https://sw-wiki.altera.com/twiki/bin/view/DesignAutomation/Syncpoint")    
    ...
    # import plugins
    plugins_dir = os.path.join(LIB, "dmx", "syncpointlib", "syncpoint_plugins")
    if not os.path.isdir(plugins_dir):
        print("Cannot find syncpoint_plugins directory!")
        sys.exit(1)
    plugins = [x[:-3] for x in os.listdir(plugins_dir) if x.endswith('.py') and x[0] != '_']
    for plugin in plugins:
        hiddenplugins = []
        if plugin not in hiddenplugins:
            __import__("dmx.syncpointlib.syncpoint_plugins." + plugin)
