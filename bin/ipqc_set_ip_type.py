...
from dmx.utillib.settings import _INFRA_PATH, _IPCATEGORY_DB, _TABLE_IP_TYPE
...
def create_table(filepath):
    ...
    # Create table
    c.execute('''CREATE TABLE {} (ipname, project, device, type, release_name)''' .format(_TABLE_IP_TYPE))
...
def update_table(filepath, chip, project, device, iptype):
    conn = sqlite3.connect(filepath)
    c = conn.cursor()

    params = (chip, project, device, iptype)
...
def delete_entry(filepath, project, device):
    conn = sqlite3.connect(filepath)
    c = conn.cursor()

    params = (project, device)
    # Insert a raw of data
    c.execute("DELETE FROM category WHERE project=? and device=?" , params)
    conn.commit()
...
class FullChip():
    ...
    def _get_devices(self):
        for product in self._family.get_products():
            if product.name in self._last_release:
                return product.name
...
def main():
    ...
    fullchips = get_released_fullchips(family)
    print(fullchips)
    list_of_fullchips = []
    ...
    if not(file_accessible(_IPCATEGORY_DB, os.F_OK)):
        create_table(_IPCATEGORY_DB)

    for chip in list_of_fullchips:

        if not(needs_update(chip.name, chip.last_release, _IPCATEGORY_DB)):
            print("No update needed for {}" .format(chip.name))
            continue

        delete_entry(_IPCATEGORY_DB, family.name, chip.device)

        conn = sqlite3.connect(_IPCATEGORY_DB)
        c = conn.cursor()
        t = (chip.name,)
        ...
        l = [(chip.name, family.name, chip.device, _FULLCHIP, chip.last_release)]

        for ip in chip.integration_ips:
            l.append((ip, family.name, chip.device, _SUBSYSTEM, chip.last_release))

        for ip in chip.ips:
            l.append((ip, family.name, chip.device, _IP, chip.last_release))
