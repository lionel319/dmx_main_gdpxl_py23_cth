...
from dmx.utillib.ipcategory import get_released_fullchips, get_ipqc_snap_path
...
class IP():
    ...
    def _get_releases(self):
        ...
        for milestone in self._milestones:
            path = os.path.join(self._ipqc_path, milestone, 'snap')
...
def create_table(filepath):
    ...
    # Create table
    c.execute('''CREATE TABLE {} (ip_id INTEGER PRIMARY KEY AUTOINCREMENT, ipname, project, release_name, milestone, report_path)''' .format(_TABLE))
...
def main():
    ...
    path = get_ipqc_snap_path(family)
    ips = os.listdir(path)
    ip_list = []
    ...
    if not(file_accessible(_SNAPS_DB, os.F_OK)):
        create_table(_SNAPS_DB)

    l = []

    for ip in ip_list:
        for release in ip.releases:
            if not(release_is_in_db(ip.name, release.name, _SNAPS_DB)):
                print("Updating {} {}" .format(ip.name, release.name))
                l.append((ip.name, family.name, release.name, release.milestone, release.report_path))

    conn = sqlite3.connect(_SNAPS_DB)
    c= conn.cursor()

    c.executemany('''INSERT INTO releases(ipname, project, release_name, milestone, report_path) VALUES (?, ?, ?, ?, ?)''' , l)
    conn.commit()
