...
class TestWaiverFile(unittest.TestCase):
    
    def setUp(self):
        self.module_path = sys.modules[WaiverFile.__module__].__file__
        ...
        self.w = WaiverFile()
        self.w.load_from_file(self.waiverfile)

        self.xascii = [[u'variant_a', u'reldoc', u'*', u'------------', u'Match Exact Error Sentance.', '/nfs/site/disks/da_scratch_1/users/yltan/depot/da/infra/dmx/main/system_tests/tnr/fake_workspace/variant_a/reldoc/tnrwaivers.csv'], [u'variant_a', u'*', u'*', u'------------', u'Match With * Wildcard.', '/nfs/site/disks/da_scratch_1/users/yltan/depot/da/infra/dmx/main/system_tests/tnr/fake_workspace/variant_a/reldoc/tnrwaivers.csv'], [u'variant_a', u'reldoc', u'*', u'------------', u'FAILED validation of /iosubsysleftnd5/cvsignoff/audit/audit.iosubsysleftnd5.cvsignoff.xml: checksum for file /iosubsysleftnd5/pnr/results/iosubsysleftnd5.pnr.pt.v (888eb90b53096765d250306a9be796f0) does not match audit requirement (992e93e8ec0e04e847fd17925d061b08).Revision #* of the file was used during checking, but an attempt was made to release revision #*.', '/nfs/site/disks/da_scratch_1/users/yltan/depot/da/infra/dmx/main/system_tests/tnr/fake_workspace/variant_a/reldoc/tnrwaivers.csv']]
    ...
    def test_020___to_ascii(self):
        ret = self.w.to_ascii()
        self.assertEqual(type(ret), type(self.xascii))

    def test_030___from_ascii(self):
        ret = self.w.from_ascii(self.xascii)
        self.assertEqual(type(ret), type(self.w))


    def test_040___is_equal(self):
        ret = self.w.is_equal(self.w)
        self.assertEqual(ret, True)

    def test_050___get_tnrwaivers_files(self):
        ret = self.w.get_tnrwaivers_files(self.wsroot, self.variant, self.libtype)
        self.assertIn('variant_a/reldoc/tnrwaivers.csv', ret[0])

    def test_060___autoload_tnr_waivers(self):
        ret = self.w.autoload_tnr_waivers(self.wsroot, self.variant, self.libtype)
